  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Tenants submit structured requests, managers triage with context, contractors get assigned, and owners only see clean approved updates."
      ...
      <div className="grid gap-5 2xl:grid-cols-[1.35fr_0.65fr]">
        <Panel>
          <SectionHeading title="Work order board" subtitle="Status tracking without spreadsheets or message threads." />
          <div className="grid gap-3 lg:grid-cols-5">
            {columns.map((column) => {
              const columnTickets = tickets.filter((ticket) => ticket.status === column);
              const fallback = columnTickets.length ? columnTickets : tickets.slice(1, 2);
                  ...
                  <div className="space-y-3">
                    {fallback.map((ticket) => (
                      <Link
                        ...
                        className="block rounded-[22px] bg-white p-4 shadow-sm transition hover:-translate-y-0.5"
                      >
        ...
        <Panel className="!bg-[#151612] text-white">
          ...
          <p className="mt-3 text-sm leading-6 text-white/68">
            ...
            {selected.property}. Owner visibility is currently off until the manager approves the
            summary.
          </p>
          ...
          <div className="mt-5 space-y-3">
            {[
              ...
              <div key={label as string} className="flex items-center gap-3 rounded-2xl bg-white/7 p-3">
                <div className={`grid size-9 place-items-center rounded-xl ${index < 2 ? "bg-[#bfe3b7] text-[#1d4628]" : "bg-white/10 text-white/70"}`}>
                  <Icon size={17} />
                </div>
